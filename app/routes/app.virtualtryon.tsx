...
export default function VirtualTryOnPage() {
  ...
  const deselectedOptions = useMemo(
    () => {
      ...
      backgrounds?.map((back: any) => {
        ...
        var backg = {
          value: back.uuid, label: `${back.name}`, media: <Thumbnail source={back.image} alt={back.name} size="small" />,
        }
        opt.push(backg);
  ...
  const store_setting = async () => {
    ...
    console.log(result.data.store_setting,"result.data.store_setting");
