...
import {
  require_jsx_runtime
} from "./chunk-QDVS4DUX.js";
import "./chunk-72MCKRY4.js";
import {
  BillingError,
  BotActivityDetected,
  CookieNotFound,
...
} from "./chunk-6GECYFU7.js";
import {
  init_esm,
  redirect2 as redirect
} from "./chunk-GMTWQ33B.js";
import {
  __commonJS,
  __toESM
...
function validateConfig(params) {
  ...
  const future = ((_b = params.future) == null ? void 0 : _b.v10_lineItemBilling) ? {
...
async function validateSessionToken(params, request2, token, { checkAudience = true } = {}) {
  ...
  try {
    ...
    logger2.debug("Session token is valid", {
      payload: JSON.stringify(payload)
    });
    return payload;
  } catch (error) {
    logger2.debug(`Failed to validate session token: ${error.message}`);
    throw respondToInvalidSessionToken({ params, request: request2, retryRequest: true });
  }
}
...
var ensureAppIsEmbeddedIfRequired = async (params, request2) => {
  ...
  if (api.config.isEmbeddedApp && url.searchParams.get("embedded") !== "1") {
    ...
    await redirectToShopifyOrAppRoot(request2, { api, logger: logger2, config });
  }
};
...
function authStrategyFactory({ strategy, ...params }) {
  ...
  async function respondToBouncePageRequest(request2) {
    ...
    if (url.pathname === config.auth.patchSessionTokenPath) {
      ...
      throw renderAppBridge({ config, logger: logger2, api }, request2);
    }
  }
  async function respondToExitIframeRequest(request2) {
    ...
    if (url.pathname === config.auth.exitIframePath) {
      ...
      logger2.debug("Rendering exit iframe page", { destination });
      throw renderAppBridge({ config, logger: logger2, api }, request2, { url: destination });
    }
  }
...
function authenticateCheckoutFactory(params) {
  return async function authenticateCheckout(request2, options = {}) {
    ...
    logger2.info("Authenticating checkout request");
    ...
    return {
      sessionToken: await validateSessionToken(params, request2, sessionTokenHeader, { checkAudience: false }),
...
function authenticateAppProxyFactory(params) {
  ...
  return async function authenticate(request2) {
    ...
    if (!await validateAppProxyHmac(params, url)) {
      logger2.info("App proxy request has invalid signature");
      throw new Response(void 0, {
        status: 400,
...
function authenticatePublicFactory(params) {
  ...
  const authenticatePublic = (request2, options) => {
    logger2.deprecated("3.0.0", "authenticate.public() will be deprecated in v3. Use authenticate.public.checkout() instead.");
    return authenticateCheckout(request2, options);
  };
...
async function triggerAfterAuthHook(params, session, request2, authStrategy) {
  ...
  if (config.hooks.afterAuth) {
    logger2.info("Running afterAuth hook");
...
var AuthCodeFlowStrategy = class {
  ...
  async authenticate(request2, sessionContext) {
    ...
    if (!session) {
      ...
      await redirectToAuthPage({ config, logger: logger2, api }, request2, shop);
  ...
  async ensureInstalledOnShop(request2) {
    ...
    if (!offlineSession) {
      ...
      if (isEmbedded) {
        ...
        throw await beginAuth({ api, config, logger: logger2 }, request2, false, shop);
      }
    }
  ...
  async handleAuthBeginRequest(request2, shop) {
    ...
    if (config.isEmbeddedApp && request2.headers.get("Sec-Fetch-Dest") === "iframe") {
      ...
      throw redirectWithExitIframe({ api, config, logger: logger2 }, request2, shop);
    } else {
      throw await beginAuth({ api, config, logger: logger2 }, request2, false, shop);
    }
  }
  ...
  async oauthCallbackError(error, request2, shop) {
    ...
    logger2.error("Error during OAuth callback", { error: error.message });
...
var TokenExchangeStrategy = class {
  ...
  async authenticate(request2, sessionContext) {
    ...
    if (!session || session.isExpired()) {
      logger2.info("No valid session found");
      logger2.info("Requesting offline access token");
